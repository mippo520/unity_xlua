...
function login:_awake()
    ...
    self:DoBindProperty(AccountDataInst.username, function (old, new)
        StorageInst:set(StorageKey.username, new)
    end)
    self:DoBindProperty(AccountDataInst.password, function (old, new)
        StorageInst:set(StorageKey.password, new)
    end)
